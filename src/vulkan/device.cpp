...
Device::Device(SDL_Window *window) : window{window} {
    createInstance();
    setupDebugMessenger();
    createSurface();
    pickPhysicalDevice();
...
void Device::createInstance() {
    ...
    appInfo.pApplicationName = "LostEngine App";
...
void Device::pickPhysicalDevice() {
    ...
    if (physicalDevice == VK_NULL_HANDLE) {
        Logger::logError("failed to find a suitable GPU!");
        throw;
    }

    vkGetPhysicalDeviceProperties(physicalDevice, &properties);
    Logger::log("Using device: " + std::string(properties.deviceName));
}
...
void Device::createSurface() {
    if (SDL_Vulkan_CreateSurface(window, instance, &surface_) == SDL_FALSE) {
        std::cout << "couldn't create a surface!" << std::endl;
    }
}
...
void Device::hasSDLRequiredInstanceExtensions() {
    ...
    Logger::logVerbose("required extensions:");
    auto requiredExtensions = getRequiredExtensions();
    for (const auto &required : requiredExtensions) {
        Logger::logVerbose("\t" + std::string(required));
        if (available.find(required) == available.end()) {
            Logger::logError("Missing required SDL extension: " + std::string(required));
            throw;
        }
    }
}
...
VkFormat Device::findSupportedFormat(
    ...
    Logger::logError("failed to find supported format!");
    throw;
}
...
void Device::createImageWithInfo(
    ...
    if (vkCreateImage(device_, &imageInfo, nullptr, &image) != VK_SUCCESS) {
        Logger::logError("failed to create image!");
        throw;
    }
    ...
    if (vkAllocateMemory(device_, &allocInfo, nullptr, &imageMemory) != VK_SUCCESS) {
        Logger::logError("failed to allocate image memory!");
        throw;
    }

    if (vkBindImageMemory(device_, image, imageMemory, 0) != VK_SUCCESS) {
        Logger::logError("failed to bind image memory!");
        throw;
    }
}
